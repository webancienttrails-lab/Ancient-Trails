...
export const ROOM_TYPES: Record<
  RoomType,
  { capacity: number; title: string; description: string; bedSummary: string }
> = {
  single: {
    capacity: 1,
...
const rateCategoryByType: Record<OccupancyRateType, PricingCategory> = {
  ADULT: "adult",
...
function roomTitle(roomType: RoomType, roomNumber: number) {
  return `Room ${roomNumber + 1} - ${ROOM_TYPES[roomType].title}`;
}
...
function createOption(
  ...
  const id = `${slugify(draft.title)}-${sequence}`;
...
function childPlan(
  ...
  if (mode === "standard") {
    return {
      guest: child,
      rate: childStandardBedRate(child, selectedDeparture),
    };
  }
